        ...
        // display error message in case an error
  		...
  		// 	This route will handle a single query to the football_players table which will retrieve the id & name for all of the football players.
			...
			app.post('/player_info/select_player', function(req, res) {
				var FootballPlayer = "SELECT * FROM football_players" //games played in the Fall 2018 Season
				var Sql = `WITH average_rushing AS (
			...
			WHERE player_info.name = '` + req.body.player_choice + `';`;
				...
				var lose = 'SELECT COUNT(*) FROM football_games a WHERE a.home_score < visitor_score;';
				db.task('get-everything', task => {
				    return task.batch([
				        task.any(FootballPlayer),
				        task.any(Sql),
				        task.any(lose)
				    ]);
				})
